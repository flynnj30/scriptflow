...
// ================================================================
// CRM SMART WORKSPACE - ENHANCED WITH FLOATING PANELS
// ================================================================
        ...
        try {
            ...
            Object.keys(parsed).forEach(key => {
                if (floatingPanels[key]) {
                    floatingPanels[key] = { ...floatingPanels[key], ...parsed[key] };
                }
            });
        } catch (e) {}
    }
...
function saveFloatingPanelState() {
    localStorage.setItem('workspace_floating_panels', JSON.stringify(floatingPanels));
}
...
function renderSmartWorkspace(container) {
        ...
        <div class="workspace-container">
            ...
            <div class="workspace-toolbar">
                ...
                <div class="workspace-actions">
                    ...
                    <button id="wsScriptPanelBtn" title="Call Script"><i class="fas fa-scroll"></i></button>
    ...
    // Create floating panels after workspace is rendered
    setTimeout(() => {
...
function createFloatingPanels() {
    ...
    createFloatingPanel('notepad', '📝 Notepad', `
        <textarea id="wsFloatingNotepad" placeholder="Quick notes...">${workspaceNotes}</textarea>
    `);
    
    // Create Call Script Panel
    createFloatingPanel('callscript', '📜 Call Script', `
        <textarea id="wsFloatingScript" placeholder="Your sales script..." readonly>${workspaceScript}</textarea>
...
function createFloatingPanel(panelId, title, contentHtml) {
    const state = floatingPanels[panelId];
    if (!state) return;
...
function setupFloatingPanelControls(panelId) {
    ...
    // Setup content-specific controls
...
function setupPanelContent(panelId) {
    ...
    switch(panelId) {
            ...
            if (scriptArea && editBtn) {
                editBtn.addEventListener('click', function() {
                    if (scriptArea.hasAttribute('readonly')) {
                        ...
                        // Sync with docked script
                        const dockedScript = document.getElementById('wsScriptArea');
                        if (dockedScript) dockedScript.value = workspaceScript;
                    }
                });
            }
            break;
        ...
        case 'calculator':
            // Calculator is functional via global function
            break;
...
// Workspace calculator global function
...
// Override makeWorkspaceDraggable to save position
...
function setupWorkspaceEvents(container) {
    ...
    notesArea.addEventListener('input', () => {
        ...
        if (floatingNote) floatingNote.value = workspaceNotes;
    });
    ...
    scriptArea.addEventListener('input', () => {
        ...
        if (floatingScript) floatingScript.value = workspaceScript;
    });
