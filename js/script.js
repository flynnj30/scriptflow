...
function parseAppointmentFromText(text, defaultDate) {
    const result = { business: '', contactName: '', role: 'Owner', phone: '', time: '', notes: '', assigned: 'Daniel', status: 'Warm-Booked', parsedDate: null };
    ...
    if (statusMatch) result.status = statusMatch[1] === 'Booked' ? 'Warm-Booked' : statusMatch[1];
...
function openSmartAddModal() {
    ...
    document.getElementById('smartParseBtn').addEventListener('click', () => {
        ...
        document.getElementById('smartPreviewContent').innerHTML = `<div style="margin-top:8px;"><div><strong>📅 Date:</strong> ${currentParsed.finalDate}</div><div><strong>🏢 Business:</strong> ${escapeHtml(currentParsed.business || '—')}</div><div><strong>👤 Contact:</strong> ${escapeHtml(currentParsed.contactName || '—')}</div><div><strong>💼 Role:</strong> ${escapeHtml(currentParsed.role || '—')}</div><div><strong>📞 Phone:</strong> ${escapeHtml(currentParsed.phone || '—')}</div><div><strong>⏰ Time:</strong> ${escapeHtml(currentParsed.time || '—')}</div><div><strong>📌 Status:</strong> ${escapeHtml(currentParsed.status || 'Warm-Booked')}</div><div><strong>👨‍💼 Assigned:</strong> ${escapeHtml(currentParsed.assigned || 'Daniel')}</div><div><strong>📝 Notes:</strong> ${escapeHtml(currentParsed.notes || '—')}</div></div>`;
    ...
    document.getElementById('smartSaveBtn').addEventListener('click', () => {
        ...
        addAppointment(currentParsed.finalDate, currentParsed.business, currentParsed.contactName, currentParsed.role, currentParsed.phone, currentParsed.time, currentParsed.notes, currentParsed.assigned, null, currentParsed.status);
        modal.remove();
...
function addAppointment(dateStr, business, contactName, role, phone, time, notes, assigned, editId = null, status = 'Warm-Booked') {
    ...
    const newAppt = { id: editId || Date.now(), business, contactName, role, phone, time, notes, assigned, status: status || 'Warm-Booked', createdAt: new Date().toISOString(), fullText: `Business: ${business}\nContact: ${contactName}\nRole: ${role}\nPhone: ${phone}\nTime: ${time}\nStatus: ${status || 'Warm-Booked'}\nNotes: ${notes}\nAssigned: ${assigned}\nDate: ${dateStr}` };
...
function renderAppointmentsList(dateStr) {
    ...
    return apptData.map(r => `<div class="appointment-item-draggable" draggable="true" data-id="${r.id}" data-date="${dateStr}" style="background:var(--bg-card); border-radius:16px; padding:16px; margin-bottom:12px; border:1px solid var(--border-color);"><div style="display:flex; justify-content:space-between; align-items:start; flex-wrap:wrap; gap:8px;"><div><strong>${escapeHtml(r.business)}</strong> <span style="color:var(--text-muted);">${escapeHtml(r.role || '')}</span></div><div><select class="status-select-calendar" data-id="${r.id}" data-date="${dateStr}" style="padding:4px 8px; border-radius:20px;">${STATUS_OPTIONS.map(s => `<option value="${s}" ${getStatus(r) === s ? 'selected' : ''}>${s}</option>`).join('')}</select><button class="action-btn-sm copy-calendar" data-id="${r.id}"><i class="fas fa-copy"></i> Copy</button><button class="action-btn-sm edit-calendar" data-id="${r.id}" data-date="${dateStr}"><i class="fas fa-edit"></i> Edit</button><button class="action-btn-sm danger delete-calendar" data-id="${r.id}" data-date="${dateStr}"><i class="fas fa-trash"></i> Delete</button></div></div><div style="margin-top:8px; font-size:0.85rem;"><div><i class="fas fa-user"></i> ${escapeHtml(r.contactName)}</div><div><i class="fas fa-phone"></i> ${escapeHtml(r.phone || 'No phone')}</div><div><i class="fas fa-clock"></i> ${escapeHtml(r.time || 'No time')}</div><div><i class="fas fa-sticky-note"></i> ${escapeHtml(r.notes || 'No notes')}</div><div><i class="fas fa-user-tie"></i> Assigned: ${escapeHtml(r.assigned || 'Unassigned')}</div></div></div>`).join('');
}
