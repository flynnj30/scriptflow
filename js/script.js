// ================================================================
// SCRIPTFLOW PRO - COMPLETE FIREBASE BACKEND (ALL FEATURES)
// ================================================================
...
// Workspace State
let workspaceTabs = [];
let workspaceActiveTabId = null;
let workspaceTabCounter = 0;
let workspaceHistory = {};
let workspaceHistoryIndex = {};
let workspaceTimerInterval = null;
let workspaceTimerSeconds = 0;
let workspaceClipboardHistory = JSON.parse(localStorage.getItem('workspace_clipboard') || '[]');
let workspaceBookmarks = JSON.parse(localStorage.getItem('workspace_bookmarks') || '[]');
let workspaceNotes = localStorage.getItem('workspace_notes') || '';
let workspaceScript = localStorage.getItem('workspace_script') || 'Hi [Name], this is [Your Name] from Regen Digital. I noticed you recently...\n\nI\'m reaching out because we help businesses like yours get more visibility online.\n\nWould you be open to a quick 5-minute chat to see if we can help?';
let workspaceView = 'browser';

const WORKSPACE_HOME = 'https://example.com';
const WORKSPACE_SEARCH = 'https://www.google.com/search?q=';
        ...
        <div class="workspace-container">
            ...
            <div class="workspace-toolbar">
                <div class="workspace-actions">
                    <button id="wsBackBtn" title="Go Back"><i class="fas fa-arrow-left"></i></button>
                    <button id="wsForwardBtn" title="Go Forward"><i class="fas fa-arrow-right"></i></button>
                ...
                <div class="workspace-actions">
                    <button id="wsToggleSidebar" title="Toggle Sidebar"><i class="fas fa-columns"></i></button>
                    ...
                    <button id="wsToggleClipboard" title="Clipboard"><i class="fas fa-clipboard-list"></i></button>
                ...
                <div class="workspace-sidebar" id="wsSidebar">
                    ...
                    <div class="workspace-sidebar-content">
                        <div class="panel-section">
                            ...
                            <textarea id="wsNotesArea" placeholder="Jot down client notes here...">${workspaceNotes}</textarea>
                        </div>
                        <div class="panel-section">
                            <h5><i class="fas fa-scroll"></i> Sales Script</h5>
                            <textarea id="wsScriptArea" placeholder="Your sales script...">${workspaceScript}</textarea>
...
function initWorkspace(container) {
    // Create initial tab
    createWorkspaceTab(WORKSPACE_HOME, true);
    
    // Setup event listeners
    setupWorkspaceEvents(container);
...
function createWorkspaceTab(url, activate = true) {
    const id = 'ws_tab_' + (++workspaceTabCounter);
    ...
    // Store tab data
    workspaceTabs.push({ id, url, title: 'Loading...' });
    workspaceHistory[id] = [url];
    workspaceHistoryIndex[id] = 0;
...
function loadWorkspaceUrl(input, tabId = workspaceActiveTabId, addToHistory = true) {
    let finalUrl = input.trim();
    if (!finalUrl || finalUrl === 'about:blank') finalUrl = WORKSPACE_HOME;
    ...
    try {
        ...
        if (/^[a-zA-Z0-9][a-zA-Z0-9-_.]*\.[a-zA-Z]{2,}/.test(finalUrl)) {
            ...
            finalUrl = WORKSPACE_SEARCH + encodeURIComponent(finalUrl);
        }
    }
    ...
    function finalizeWorkspaceLoad(tabId, finalUrl, addToHistory) {
        ...
        if (addToHistory && workspaceHistory[tabId]) {
            const history = workspaceHistory[tabId];
            const index = workspaceHistoryIndex[tabId];
            if (history[history.length - 1] !== finalUrl) {
                workspaceHistory[tabId] = history.slice(0, index + 1);
                workspaceHistory[tabId].push(finalUrl);
                workspaceHistoryIndex[tabId] = workspaceHistory[tabId].length - 1;
            }
        }
        
        if (tabId === workspaceActiveTabId) {
            updateWorkspaceNavButtons();
        }
    }
}
...
function updateWorkspaceNavButtons() {
    ...
    const history = workspaceHistory[tab.id] || [];
    const index = workspaceHistoryIndex[tab.id] || 0;
    
    document.getElementById('wsBackBtn').disabled = index <= 0;
    document.getElementById('wsForwardBtn').disabled = index >= history.length - 1;
}
...
function updateWorkspaceBookmarkButton(url) {
    ...
    if (!btn) return;
    if (workspaceBookmarks.includes(url)) {
...
function setupWorkspaceEvents(container) {
    // URL Input
    const urlInput = document.getElementById('wsUrlInput');
    urlInput.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            loadWorkspaceUrl(urlInput.value);
        }
    });
    ...
    document.getElementById('wsBackBtn').addEventListener('click', () => {
        ...
        const index = workspaceHistoryIndex[tab.id] || 0;
        if (index > 0) {
            workspaceHistoryIndex[tab.id] = index - 1;
            loadWorkspaceUrl(history[index - 1], workspaceActiveTabId, false);
        }
    });
    ...
    document.getElementById('wsReloadBtn').addEventListener('click', () => {
        const tab = workspaceTabs.find(t => t.id === workspaceActiveTabId);
        if (tab) loadWorkspaceUrl(tab.url, workspaceActiveTabId, false);
    });
    
    document.getElementById('wsHomeBtn').addEventListener('click', () => {
        loadWorkspaceUrl(WORKSPACE_HOME);
    });
    ...
    document.getElementById('wsBookmarkBtn').addEventListener('click', () => {
        const tab = workspaceTabs.find(t => t.id === workspaceActiveTabId);
        if (!tab) return;
        const idx = workspaceBookmarks.indexOf(tab.url);
        if (idx > -1) {
            workspaceBookmarks.splice(idx, 1);
        } else {
            workspaceBookmarks.push(tab.url);
        }
        localStorage.setItem('workspace_bookmarks', JSON.stringify(workspaceBookmarks));
        updateWorkspaceBookmarkButton(tab.url);
    ...
    notesArea.addEventListener('input', () => {
        workspaceNotes = notesArea.value;
        localStorage.setItem('workspace_notes', workspaceNotes);
    });
    
    // Script
    const scriptArea = document.getElementById('wsScriptArea');
    scriptArea.addEventListener('input', () => {
        workspaceScript = scriptArea.value;
        localStorage.setItem('workspace_script', workspaceScript);
    });
    ...
    document.getElementById('wsScriptEditBtn').addEventListener('click', function() {
        ...
        if (area.hasAttribute('readonly')) {
            ...
            localStorage.setItem('workspace_script', area.value);
        }
    });
    // Initially set script as readonly
    scriptArea.setAttribute('readonly', 'readonly');
    
    // Calculator
    document.getElementById('wsToggleCalc').addEventListener('click', () => {
        toggleWorkspaceFloatingPanel('calcPanel');
    });
    
    // Timer
    document.getElementById('wsToggleTimer').addEventListener('click', () => {
        toggleWorkspaceFloatingPanel('timerPanel');
    });
    
    // Clipboard
    document.getElementById('wsToggleClipboard').addEventListener('click', () => {
        toggleWorkspaceFloatingPanel('clipboardPanel');
        renderWorkspaceClipboard();
    });
...
function toggleWorkspaceFloatingPanel(panelId) {
    const panelMap = {
        'calcPanel': { element: null, html: `
            <div class="panel-header" style="cursor:move;">
                <h5>🧮 Calculator</h5>
                <button class="ws-panel-close">✖</button>
            </div>
            <div class="panel-body">
                <input type="text" class="workspace-calc-display" id="wsCalcDisplay" disabled>
        ...
        `, setup: setupWorkspaceCalc },
        'timerPanel': { element: null, html: `
            <div class="panel-header" style="cursor:move;">
                <h5>⏱️ Call Timer</h5>
                <button class="ws-panel-close">✖</button>
            </div>
            <div class="panel-body">
                <div class="workspace-timer-display" id="wsTimerDisplay">00:00:00</div>
                <div class="workspace-timer-controls">
                    <button class="timer-start" id="wsTimerStart">Start</button>
                    <button class="timer-pause" id="wsTimerPause">Pause</button>
                    <button class="timer-reset" id="wsTimerReset">Reset</button>
                </div>
            </div>
        `, setup: setupWorkspaceTimer },
        'clipboardPanel': { element: null, html: `
            <div class="panel-header" style="cursor:move;">
                <h5>📋 Clipboard History</h5>
                <button class="ws-panel-close">✖</button>
            </div>
            <div class="panel-body">
                <button class="workspace-clipboard-capture" id="wsCaptureClipboard">📥 Capture Current Clipboard</button>
                <ul class="workspace-clipboard-list" id="wsClipboardList"></ul>
            </div>
        `, setup: setupWorkspaceClipboard }
    };
    ...
    if (panelInfo.element) {
        ...
        panelInfo.element = null;
        return;
    }
    
    // Create panel
    const panel = document.createElement('div');
    panel.className = 'workspace-floating-panel visible';
    panel.id = `ws_${panelId}`;
    panel.innerHTML = panelInfo.html;
    ...
    // Setup panel
    if (panelInfo.setup) panelInfo.setup(panel);
    
    // Close button
    panel.querySelector('.ws-panel-close').addEventListener('click', () => {
        panel.remove();
        panelInfo.element = null;
    });
    ...
    makeWorkspaceDraggable(panel, header);
}

function makeWorkspaceDraggable(panel, header) {
...
function setupWorkspaceCalc(panel) {
    window.workspaceCalcPress = function(val) {
        const d = document.getElementById('wsCalcDisplay');
        if (!d) return;
        if (val === 'C') {
            d.value = '';
        } else if (val === '=') {
...
function setupWorkspaceTimer(panel) {
    let timerInterval = null;
    let seconds = 0;
    const display = document.getElementById('wsTimerDisplay');
    ...
    document.getElementById('wsTimerStart').addEventListener('click', () => {
    ...
    document.getElementById('wsTimerPause').addEventListener('click', () => {
        clearInterval(timerInterval);
    });
    
    document.getElementById('wsTimerReset').addEventListener('click', () => {
        clearInterval(timerInterval);
        seconds = 0;
        updateDisplay();
    });
...
function setupWorkspaceClipboard(panel) {
    renderWorkspaceClipboard();
    
    document.getElementById('wsCaptureClipboard').addEventListener('click', async () => {
        try {
            const text = await navigator.clipboard.readText();
            if (text && !workspaceClipboardHistory.includes(text)) {
                workspaceClipboardHistory.unshift(text);
                if (workspaceClipboardHistory.length > 20) workspaceClipboardHistory.pop();
                localStorage.setItem('workspace_clipboard', JSON.stringify(workspaceClipboardHistory));
                renderWorkspaceClipboard();
                document.getElementById('wsStatusText').textContent = '✅ Clipboard captured';
                setTimeout(() => document.getElementById('wsStatusText').textContent = '✅ Ready', 2000);
            }
        } catch(e) {
...
function renderWorkspaceClipboard() {
    const list = document.getElementById('wsClipboardList');
    if (!list) return;
    list.innerHTML = '';
    workspaceClipboardHistory.forEach(text => {
        ...
        li.addEventListener('click', async () => {
            try {
                ...
                document.getElementById('wsStatusText').textContent = '✅ Copied to clipboard';
                setTimeout(() => document.getElementById('wsStatusText').textContent = '✅ Ready', 2000);
            } catch(e) {}
        });
        list.appendChild(li);
        ...
        try {
            ...
            if (e.key >= '1' && e.key <= '9' && !isEditing && !e.target.matches('textarea,input')) {
                ...
                if (fp && fp.style.display === 'block') {
                    // Check if workspace is active - if so, don't intercept numbers
                    const container = document.getElementById('featurePanelBody');
                    if (container && container.querySelector('.workspace-container')) return;
                }
                e.preventDefault();
